...
class SPMamba(nn.Module):
    def __init__(self, cfg, in_dim):
        ...
        self.global_mamba = SS2D(d_model=in_dim, d_state=16, expand=2, dropout=drop_rate)
        self.local_mamba = SS2D_local(d_model=in_dim, d_state=16, expand=2, dropout=drop_rate)
        
        self.dwt = DWTForward(J=1, mode="zero", wave="haar")
        self.idwt = DWTInverse(mode="zero", wave="haar")
        self.fuse_weight = nn.Parameter(torch.zeros(2))
    ...
    def forward(self, feas, ds=False, scale=0.5):
        bs, _, h, w = feas.shape
        if ds:
            feas_proc = nn.AdaptiveMaxPool2d((int(h * scale), int(w * scale)))(feas)
        else:
            feas_proc = feas
            
        global_in = rearrange(feas_proc, "b c h w -> b h w c").contiguous()
        global_out = self.global_mamba(global_in)
        global_out = rearrange(global_out, "b h w c -> b c h w").contiguous()

        yl, yh = self.dwt(feas_proc)
        yl_in = rearrange(yl, "b c h w -> b h w c").contiguous()
        
        if min(yl_in.shape[1], yl_in.shape[2]) >= 4:
            yl_out = self.local_mamba(yl_in)
        else:
            yl_out = yl_in
            
        yl_out = rearrange(yl_out, "b h w c -> b c h w").contiguous()
        wave_out = self.idwt((yl_out, yh))
        
        if wave_out.shape[-2:] != feas_proc.shape[-2:]:
            wave_out = F.interpolate(wave_out, size=feas_proc.shape[-2:], mode="nearest")

        weights = torch.softmax(self.fuse_weight, dim=0)
        attn_out = weights[0] * global_out + weights[1] * wave_out
        
        if ds:
            attn_out = F.interpolate(attn_out, size=(h, w), mode="nearest")
